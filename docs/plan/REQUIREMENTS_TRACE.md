...
| LLM providers | OpenAI/Claude/Gemini, API key auth, OpenAI OAuth, usage/cost/latency records, monthly limits | API-key provider adapter foundation, provider availability, usage/cost/latency records, missing-credential handling, and cost guard exist; OpenAI OAuth remains a later explicit connection flow | M8 accepted; OAuth follow-up later |
